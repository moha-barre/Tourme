...
import { Trophy, Users, Calendar, Award, Copy, RefreshCw, CheckCircle, XCircle } from "lucide-react";
...
export default function TournamentDetailPage() {
  ...
  const fetchTournamentData = async () => {
    try {
      ...
      const { data: participantsData, error: participantsError } =
        await supabase.from("participants").select("*").eq("tournament_id", id);
  ...
  // Helper: Check if user is already a participant
  ...
  // Helper: Get participant object for current user
  const myParticipant = participants.find((p) => p.user_id === user?.id);

  // Helper: Copy tournament link
  ...
  // Join logic with display_name fix and DB constraint check
  const handleJoinTournament = async () => {
    ...
    try {
      ...
      // Double-check if tournament is full (race condition)
      ...
      if (typeof count === "number" && count >= tournament.max_participants) {
        setError("Tournament is already full.");
        setJoining(false);
        await fetchTournamentData();
        return;
      }

      // Insert participant with display_name
      const displayName =
        user.username ||
        user.email?.split("@")[0] ||
        teamName ||
    ...
    return (
      <div className="max-w-7xl mx-auto">
        <EmptyState
          ...
          description={
            error || "The tournament you are looking for does not exist."
          }
          action={{
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-6 lg:space-y-0">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  <Link to={`/tournaments/brackets/${id}`}>
                    <Button
                      variant="outline"
                      size="md"
                      className="w-full sm:w-auto"
                ...
                <div className="space-y-3">
                  {participants.map((participant) => {
                    const isParticipantAdmin =
                      participant.user_id === tournament.admin_id;
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500">
                              {participant.team_name && (
                                <span className="mr-2">
                                  <span className="font-medium">Team:</span> {participant.team_name}
                                </span>
                              )}
        ...
        <div className="mt-8">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div className="p-6">
              ...
              {/* Optionally, you could add a bracket visualization here */}
            </div>
          </div>
        </div>
      )}
